/*
 * Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
 * This product includes software developed at Datadog (https://www.datadoghq.com/).
 * Copyright 2019-Present Datadog, Inc.
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package datadog

import (
	"encoding/json"
)

// UserJSONAPIResponse The JSON API Schema User response.
type UserJSONAPIResponse struct {
	Attributes *User `json:"attributes,omitempty"`
	// A unique identifier that represents the user.
	Id            *string                           `json:"id,omitempty"`
	Relationships *UserJSONAPIResponseRelationships `json:"relationships,omitempty"`
	// JSONAPI Model Type
	Type *string `json:"type,omitempty"`
}

// NewUserJSONAPIResponse instantiates a new UserJSONAPIResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUserJSONAPIResponse() *UserJSONAPIResponse {
	this := UserJSONAPIResponse{}
	var type_ string = "users"
	this.Type = &type_
	return &this
}

// NewUserJSONAPIResponseWithDefaults instantiates a new UserJSONAPIResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUserJSONAPIResponseWithDefaults() *UserJSONAPIResponse {
	this := UserJSONAPIResponse{}
	var type_ string = "users"
	this.Type = &type_
	return &this
}

// GetAttributes returns the Attributes field value if set, zero value otherwise.
func (o *UserJSONAPIResponse) GetAttributes() User {
	if o == nil || o.Attributes == nil {
		var ret User
		return ret
	}
	return *o.Attributes
}

// GetAttributesOk returns a tuple with the Attributes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserJSONAPIResponse) GetAttributesOk() (*User, bool) {
	if o == nil || o.Attributes == nil {
		return nil, false
	}
	return o.Attributes, true
}

// HasAttributes returns a boolean if a field has been set.
func (o *UserJSONAPIResponse) HasAttributes() bool {
	if o != nil && o.Attributes != nil {
		return true
	}

	return false
}

// SetAttributes gets a reference to the given User and assigns it to the Attributes field.
func (o *UserJSONAPIResponse) SetAttributes(v User) {
	o.Attributes = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *UserJSONAPIResponse) GetId() string {
	if o == nil || o.Id == nil {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserJSONAPIResponse) GetIdOk() (*string, bool) {
	if o == nil || o.Id == nil {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *UserJSONAPIResponse) HasId() bool {
	if o != nil && o.Id != nil {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *UserJSONAPIResponse) SetId(v string) {
	o.Id = &v
}

// GetRelationships returns the Relationships field value if set, zero value otherwise.
func (o *UserJSONAPIResponse) GetRelationships() UserJSONAPIResponseRelationships {
	if o == nil || o.Relationships == nil {
		var ret UserJSONAPIResponseRelationships
		return ret
	}
	return *o.Relationships
}

// GetRelationshipsOk returns a tuple with the Relationships field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserJSONAPIResponse) GetRelationshipsOk() (*UserJSONAPIResponseRelationships, bool) {
	if o == nil || o.Relationships == nil {
		return nil, false
	}
	return o.Relationships, true
}

// HasRelationships returns a boolean if a field has been set.
func (o *UserJSONAPIResponse) HasRelationships() bool {
	if o != nil && o.Relationships != nil {
		return true
	}

	return false
}

// SetRelationships gets a reference to the given UserJSONAPIResponseRelationships and assigns it to the Relationships field.
func (o *UserJSONAPIResponse) SetRelationships(v UserJSONAPIResponseRelationships) {
	o.Relationships = &v
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *UserJSONAPIResponse) GetType() string {
	if o == nil || o.Type == nil {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserJSONAPIResponse) GetTypeOk() (*string, bool) {
	if o == nil || o.Type == nil {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *UserJSONAPIResponse) HasType() bool {
	if o != nil && o.Type != nil {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *UserJSONAPIResponse) SetType(v string) {
	o.Type = &v
}

func (o UserJSONAPIResponse) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Attributes != nil {
		toSerialize["attributes"] = o.Attributes
	}
	if o.Id != nil {
		toSerialize["id"] = o.Id
	}
	if o.Relationships != nil {
		toSerialize["relationships"] = o.Relationships
	}
	if o.Type != nil {
		toSerialize["type"] = o.Type
	}
	return json.Marshal(toSerialize)
}

type NullableUserJSONAPIResponse struct {
	value *UserJSONAPIResponse
	isSet bool
}

func (v NullableUserJSONAPIResponse) Get() *UserJSONAPIResponse {
	return v.value
}

func (v *NullableUserJSONAPIResponse) Set(val *UserJSONAPIResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableUserJSONAPIResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableUserJSONAPIResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUserJSONAPIResponse(val *UserJSONAPIResponse) *NullableUserJSONAPIResponse {
	return &NullableUserJSONAPIResponse{value: val, isSet: true}
}

func (v NullableUserJSONAPIResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUserJSONAPIResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
